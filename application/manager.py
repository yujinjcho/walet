...
def handle_account(access_token, refresh_token):
    ...
    account_ids = data_layer.account_by_email(email)
    ...
    if account_ids:
        data_layer.update_account(account_ids[0], encrypted_token, encrypted_refresh_token)
        return account_ids[0]

    data_layer.create_account(email, encrypted_token, encrypted_refresh_token)
    new_account_ids = data_layer.account_by_email(email)
    account_id = new_account_ids[0]

    data_layer.create_tag(account_id, 'exclude')
    return account_id

def get_plaid_accounts(account_id):
    tokens = [token[1] for token in data_layer.access_tokens(account_id)]
...
def create_plaid_accounts(account_id, public_token):
    ...
    return data_layer.save_access_token(encrypted_access_token, item_id, account_id)

def get_transactions(account_id, month):
    # {item_id: token}
    tokens = {token[0]: token[1] for token in data_layer.access_tokens(account_id)}

    transactions = []
    error = None
    for item_id, token in tokens.items():

        stored_transactions = [x[0] for x in data_layer.get_transactions(account_id, item_id, month)]
        if stored_transactions:
            print(f"retrieving stored transactions for item_id: {item_id}")
            data_layer.update_plaid_categories(helper.extract_categories(stored_transactions), account_id)
            ...
            try:
                ...
                data_layer.update_transactions(db_transactions)
                data_layer.update_plaid_categories(helper.extract_categories(item_transactions), account_id)
...
def get_budgets(account_id, month, year):
    budgets = data_layer.budgets(account_id, month, year)
    if not budgets:
        # using -1 to represent default
        budgets = data_layer.budgets(account_id, "-1", year)
...
def update_budgets(account_id, budget_update_request):
    ...
    data_layer.delete_budgets(account_id, month, year)
    ...
    return data_layer.create_budgets(updated_budgets)
...
def handle_webhook(webhook):
    ...
    if code == "DEFAULT_UPDATE":
        tokens = data_layer.access_token_by_item_id(item_id)
        if tokens:
            ...
            data_layer.update_plaid_categories(helper.extract_categories(transactions), account_id)
            result = data_layer.update_transactions(db_transactions)
            print(f"updated rows: {result}")

    elif code == "TRANSACTIONS_REMOVED":
        result = data_layer.delete_transactions(item_id, webhook['removed_transactions'])
