...
def get_transactions(account_id, month):
    ...
    for item_id, token in tokens.items():
        ...
        if stored_transactions:
            print(f"retrieving stored transactions for item_id: {item_id} and month: {month}")
            ...
            try:
                ...
                update_transactions(account_id, item_id, item_transactions)
...
def get_annual_transactions(account_id, year):
    # {item_id: token}
    tokens = {token[0]: token[1] for token in data.access_tokens(account_id)}
    ...
    for item_id, token in tokens.items():
        ...
        stored_transactions = [x[0] for x in data.get_annual_transactions(account_id, item_id, year)]
        if stored_transactions:
            print(f"retrieving stored transactions for item_id: {item_id} and for year: {year}")
            data.update_plaid_categories(helper.extract_categories(stored_transactions), account_id)
            transactions.extend(stored_transactions)
        else:
            try:
                item_transactions = plaid.annual_transactions(year, token)

                db_transactions = [
                    (x['transaction_id'], account_id, json.dumps(x), x['date'], item_id)
                    for x in item_transactions
                ]
                print(f"persisting {len(item_transactions)} transactions for year: {year}, item_id: {item_id}")
...
def handle_webhook(webhook):
    ...
    if code == "DEFAULT_UPDATE":
        ...
        if tokens:
            ...
            transactions = plaid.recent_transactions(access_token)
            update_transactions(account_id, item_id, transactions)

    elif code == "HISTORICAL_UPDATE":
        tokens = data.access_token_by_item_id(item_id)
        if tokens:
            access_token, account_id = tokens[0]
            transactions = plaid.recent_transactions(access_token, 'year')
            update_transactions(account_id, item_id, transactions)
...
def update_transactions(account_id, item_id, transactions):
    db_transactions = [
        (x['transaction_id'], account_id, json.dumps(x), x['date'], item_id)
